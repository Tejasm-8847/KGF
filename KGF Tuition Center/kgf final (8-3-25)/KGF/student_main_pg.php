...
// Fetch student name and other info from the database
$studentID = $_SESSION['student_id'];  // Make sure session has the correct student ID
$query = "SELECT Name, TeacherID, TotalClasses, ClassesAttended FROM Student WHERE StudentID = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $studentID);
$stmt->execute();
$stmt->bind_result($studentName, $teacherID, $totalClasses, $classesAttended);
$stmt->fetch();
$stmt->close();
...
<head>
  ...
  <title>KGF Tuitions - Student Dashboard</title>
...
<body>
  ...
  <div id="topbar" style="background-color: #ff4925;">
    ...
    <div class="user-tab" id="user-tab">
      <div class="user-name"><?php echo htmlspecialchars($studentName); ?></div> <!-- Display student's name -->
  ...
  <div class="main-wrapper">
    ...
    <!-- Main Content -->
    <div id="main-content">
      <h1>Welcome, <?php echo htmlspecialchars($studentName); ?>!</h1> <!-- Display student's name -->
      
      <!-- Dashboard Boxes -->
      <div class="dashboard-boxes">
        <a href="student_attendence_pg.php">
          <div class="dashboard-box">
            <i class="bi bi-check-circle-fill" style="color: #ff4925;"></i>
            <h3>Attendance</h3>
            <p>Track your attendance records.</p>
          </div>
        </a>

        <a href="student_report_pg.php">
          <div class="dashboard-box">
            <i class="bi bi-bar-chart-fill" style=" color: #ff4925;"></i>
            <h3>Performance</h3>
            <p>View your performance scores and progress.</p>
          </div>
        </a>

        <a href="student_notification_pg.php">
          <div class="dashboard-box">
